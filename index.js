...
import { startGameMonitoring, getGameStatus } from './src/gameDetection.js';

import { startDataSync, stopDataSync } from './src/dataService.js';
...
// Menu elements
const menuUserFullnameSpan = document.getElementById('menu-user-fullname');
const menuUserRegimentSpan = document.getElementById('menu-user-regiment');
const serviceMenuBtn = document.getElementById('service-menu-btn');
const announcementsMenuBtn = document.getElementById('announcements-menu-btn');
const confidentialMenuBtn = document.getElementById('confidential-menu-btn');
const adminMenuBtn = document.getElementById('admin-menu-btn');
const menuLogoutBtn = document.getElementById('menu-logout-btn');

// Back buttons
const backToMenuFromServiceBtn = document.getElementById('back-to-menu-from-service-btn');
const backToMenuFromAnnouncementsBtn = document.getElementById('back-to-menu-from-announcements-btn');
const backToMenuFromAdminBtn = document.getElementById('back-to-menu-from-admin-btn');
...
const announcementConfidential = document.getElementById('announcement-confidential');
...
function initApp() {
  ...
  startDataSync(); // Start data synchronization
}
...
function addEventListeners() {
  ...
  announcementsMenuBtn?.addEventListener('click', () => {
    mainMenuSection.classList.add('hidden');
    announcementsSection.classList.remove('hidden');
    loadAnnouncements();
  });
  
  confidentialMenuBtn?.addEventListener('click', () => {
  ...
  // Back buttons
  backToMenuFromServiceBtn?.addEventListener('click', showMainMenu);
  backToMenuFromAnnouncementsBtn?.addEventListener('click', showMainMenu);
  backToMenuFromAdminBtn?.addEventListener('click', showMainMenu);
  ...
  saveAnnouncementBtn?.addEventListener('click', () => {
    ...
    const isConfidential = announcementConfidential.checked;
    ...
    const result = saveAnnouncement(title, content, isConfidential, user.fullname);
    
    if (result.success) {
      alert('Annonce publiée avec succès');
      announcementTitle.value = '';
      announcementContent.value = '';
      announcementConfidential.checked = false;
      loadAnnouncements();
    } else {
      alert(result.message || 'Erreur lors de la publication de l\'annonce');
    }
  });
}

// Update service controls based on service status
...
function handleLogout() {
  ...
  if (user) {
    ...
    // Clear intervals
    clearInterval(serviceTimer);
    stopDataSync(); // Stop data synchronization
...
function handleResetUserHours() {
  ...
  if (confirm(`Êtes-vous sûr de vouloir réinitialiser les heures de service de ${selectedUsername}?`)) {
    const result = resetUserHours(selectedUsername, currentUser);
    if (result.success) {
      alert(result.message);
      loadUserHoursForAdmin(selectedUsername);
      loadAllUsersHours();
...
function handleResetAllHours() {
  ...
  if (currentUser.role !== 'admin') {
    alert('Seuls les administrateurs peuvent réinitialiser toutes les heures');
    return;
  }
  
  if (confirm('Êtes-vous sûr de vouloir réinitialiser les heures de service de TOUS les utilisateurs?')) {
    const result = resetAllHours(currentUser);
    if (result.success) {
      alert(result.message);
      loadUsersForAdmin();
      loadAllUsersHours();
...
function handleResetRegimentHours(regiment) {
  ...
  if (confirm(`Êtes-vous sûr de vouloir réinitialiser les heures de service du régiment ${regiment}?`)) {
    const result = resetRegimentHours(regiment, currentUser);
    if (result.success) {
      alert(result.message);
      loadUsersForAdmin();
      loadAllUsersHours();
...
function handleDeleteUser() {
  ...
  if (confirm(`Êtes-vous sûr de vouloir supprimer le compte de ${selectedUsername}? Cette action est irréversible.`)) {
    const result = deleteUser(selectedUsername, currentUser);
    if (result.success) {
      alert(result.message);
      loadUsersForAdmin();
...
function showMainMenu() {
  ...
  // Update menu info
  menuUserFullnameSpan.textContent = user.fullname;
  menuUserRegimentSpan.textContent = user.regiment;
  
  // Show/hide admin button
  adminMenuBtn.classList.toggle('hidden', !isAdmin(user) && !isModerator(user));
...
function updateServiceDashboard() {
  const user = getCurrentUser();
  if (!user) return;
  ...
  // Update service controls based on status
  updateServiceControls();
}
...
function loadAnnouncements() {
  ...
  // Show/hide announcement form for admins/mods
  const announcementForm = document.getElementById('announcement-form');
  announcementForm.classList.toggle('hidden', !isAdmin(user) && !isModerator(user));
  
  if (announcements.length === 0) {
    announcementsList.innerHTML = '<p class="info-text">Aucune annonce disponible</p>';
    return;
  }
  ...
  announcements.forEach(announcement => {
    // Skip confidential announcements for regular users
    if (announcement.isConfidential && !isAdmin(user) && !isModerator(user)) {
      return;
    }
    ...
    item.innerHTML = `
      <div class="announcement-header">
        <span class="announcement-title">
          ${announcement.title}
          ${announcement.isConfidential ? '<span class="confidential-badge">CONFIDENTIEL</span>' : ''}
        </span>
        <span class="announcement-meta">
          Par ${announcement.author} le ${new Date(announcement.timestamp).toLocaleDateString('fr-FR')}
        </span>
      </div>
  ...
  sortedRegiments.forEach(regiment => {
    ...
    regimentHeader.innerHTML = `
      ...
      ${isAdmin(getCurrentUser()) ? 
        `<button class="military-btn danger" onclick="handleResetRegimentHours('${regiment}')">
